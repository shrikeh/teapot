<?php
/**
 * Interface representing extended HTTP status codes for Cloudflare. These codes are
...
interface Cloudflare
{
     ...
     * reverse proxies to signal an "unknown connection issue between CloudFlare
     * and the origin web server" to a client in front of the proxy.
     *
     ...
     * respond to all requests properly. The origin web server is not functioning
     * consistently for each request. Contact your hosting provider to identify
     ...
     *
     * The most common cause is that the DNS setting has changed. Sometimes, hosting
     * providers update the origin IP information for their customers. If this is
     * the case, you need to make sure the new origin IP address for your A record
     * is reflected in your CloudFlare DNS Settings page.
     *
    ...
    /**
     * This status code is not specified in any RFCs, but is used by Cloudflare's
     * reverse proxies to signal a network read timeout behind the proxy to a
     ...
     * likely cause is either an overloaded background task, database or application
     * (i.e. such as Wordpress), over stressing the resources on your machine. The
     * database is often the cause.
     *
     * The difference between a 522 and 524 error is that with a 522 error, the
